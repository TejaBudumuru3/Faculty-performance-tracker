...
int patentsLog=0;
    ...
    if (user == null) {
        ...
        try {
            ...
            String TaskIncompleteQ = "select count(tokenid) as TaskCount from token where facultyid = ? and month(date) = month(curdate()) and year(date) = year(curdate()) and status = 0";
            ...
            String TaskAcceptedQ = "select count(tokenid) as TaskCount from token where facultyid = ? and month(date) = month(curdate()) and year(date) = year(curdate()) and status = 1";
            ...
            String TaskCompleteQ = "select count(tokenid) as TaskCount from token where facultyid = ? and month(date) = month(curdate()) and year(date) = year(curdate()) and status = 2";
            ...
            // Query 5 : score
            String scoreQ = "select sum(LogCredit) AS lscore from log where facultyId = ? and month(date) = month(curdate()) and year(date) = year(curdate())";
            ...
            String patentsLogQ = "SELECT  sum(PublicationsPublished) AS Publications, sum(PatentsGained) AS Patents FROM log where FacultyID = ? and month(date) = month(curdate()) and year(date) = year(curdate())";
    ...
    <!-- Include toastr CSS -->
            ...
            <script>
                document.addEventListener("DOMContentLoaded", function() {
                    ...
                    document.getElementById("greeting").innerText = "Welcome, " + UserName + " (Faculty)!";
